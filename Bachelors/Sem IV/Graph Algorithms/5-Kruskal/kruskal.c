//C Program to find the Minimum Spanning Tree using Kruskal's Algorithm and Adjacency Matrix
#include <stdio.h>
#include <stdlib.h>
#define datatype int
...
void kruskal(datatype **edge,int m,int n)
{
	int i,j,k,r,s,tot=0,markr,marks;
	...
	for(i=0,k=0;i<n-1;k++)
	{
		r=edge[k][1];markr=mark[r];
		s=edge[k][2];marks=mark[s];
		if(mark[r]!=mark[s])
		{
			tot=tot+edge[k][0];
